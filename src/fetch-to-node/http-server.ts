 ...
 */
export class FetchServerResponse
  extends FetchOutgoingMessage
  implements ServerResponse
{
  ...
  constructor(req: IncomingMessage, options?: OutgoingMessageOptions) {
    ...
    this.fetchResponse = new Promise<Response>((resolve) => {
      let finished = false;
      this.on("finish", () => {
        finished = true;
      });
...
export function toReqRes(req: Request, options?: ToReqResOptions): ReqRes {
  const {
    createIncomingMessage = () => new FetchIncomingMessage(),
    createServerResponse = (incoming: FetchIncomingMessage) =>
      new FetchServerResponse(incoming),
    ctx,
  } = options ?? {};
...
export function toFetchResponse(res: ServerResponse) {
  if (!(res instanceof FetchServerResponse)) {
    throw new Error(
      "toFetchResponse must be called on a ServerResponse generated by toReqRes"
    );
  }
